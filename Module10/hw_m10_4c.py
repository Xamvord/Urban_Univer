        ...
        with self.tables_lock:
            ...
            for t in self.tables:
                if not t.is_busy:
                    ...
                    break
        if table:
            print(Fore.MAGENTA + Style.BRIGHT + f"Метрдотель: 'Я вас категорически приветствую, посетитель номер {customer_id}! Прошу проследовать за столик номер {table.number}'")
            customer = Customer(customer_id, self, table)
            self.active_threads.append(customer)
            customer.start()
        else:
            print(Fore.RED + f"Метрдотель: 'Я дико извиняюсь, посетитель номер {customer_id}, но свободных столиков на данный момент нету, прошу вас немного подождать!'")
